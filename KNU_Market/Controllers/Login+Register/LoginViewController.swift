...
extension LoginViewController {
    ...
    func initializeTextFields() {
        ...
        pwImage = pwImage?.scalePreservingAspectRatio(targetSize: CGSize(width: 25, height: 25))
        
        let idLeftImageView = UIImageView(frame: CGRect(x: 10, y: 10, width: 20, height: 20))
        idLeftImageView.image = idImage
        idTextField.leftView = idLeftImageView
        idTextField.leftViewMode = .always
        
        
        let pwLeftImageView = UIImageView(frame: CGRect(x: 10, y: 10, width: 20, height: 20))
